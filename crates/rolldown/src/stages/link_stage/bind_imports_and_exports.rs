...
impl LinkStage<'_> {
  ...
  /// export of a namespace-consumed module (`import * as ns` or a dynamic import), keyed by each
  /// export's canonical symbol.
  ///
  ...
  /// definer that only the namespace observes. `collect_frozen_reexport_usage` still gates every
  /// recorded path on the canonical symbol's actual usedness, so a namespace tree-shaking drops
  /// retains nothing here.
