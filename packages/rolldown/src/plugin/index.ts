...
export interface HotUpdateOptions {
  ...
  modules: string[];
  /**
   * Read the changed file's current content. An empty read during an
   * in-progress editor save is retried briefly (same behavior as Vite).
   * Rejects if the file no longer exists.
   */
  read: () => Promise<string>;
}
